# Assemble GEVR Beta vr445.2 staging folder (BYO-ROM).
# Point release: KEEP stack (#74/#75/#82/gun origin/#84 walk-run/SKYWORLD/ANIMFRAMES)
# on vr445.1 cadence. After -Vr: pack from getv\build-windows into out\vr445.2-staging (+ zip).
...
Copy-Item -LiteralPath (Join-Path $templates "Start-GEVR-vr445.2.bat") -Destination (Join-Path $OutDir "Start-GEVR.bat") -Force
Copy-Item -LiteralPath (Join-Path $templates "gevr-vr445.2-boot.cmd") -Destination (Join-Path $OutDir "gevr-vr445.2-boot.cmd") -Force
Copy-Item -LiteralPath (Join-Path $templates "RELEASE-NOTES-vr445.2.txt") -Destination (Join-Path $OutDir "RELEASE-NOTES.txt") -Force
