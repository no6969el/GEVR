# Assemble GEVR Beta vr445.1 staging folder (BYO-ROM).
# Point release: AI rifle fire cadence / weapon-prop 64-bit fix (#84) on vr445.
# After -Vr: pack from getv\build-windows into out\vr445.1-staging (+ zip).
...
Copy-Item -LiteralPath (Join-Path $templates "Start-GEVR-vr445.1.bat") -Destination (Join-Path $OutDir "Start-GEVR.bat") -Force
Copy-Item -LiteralPath (Join-Path $templates "gevr-vr445.1-boot.cmd") -Destination (Join-Path $OutDir "gevr-vr445.1-boot.cmd") -Force
Copy-Item -LiteralPath (Join-Path $templates "RELEASE-NOTES-vr445.1.txt") -Destination (Join-Path $OutDir "RELEASE-NOTES.txt") -Force
