# Assemble GEVR Beta vr445 staging folder (BYO-ROM).
# Small tag: ADS crouch package + DUALFIRE C-default + Hertz follow harden +
# monitor-on while VR. Diagnostics bats copied into the zip (not separate GH assets).
# After -Vr: pack from getv\build-windows into out\vr445-staging (+ zip).
...
# Ship layout
Copy-Item -LiteralPath (Join-Path $templates "Start-GEVR-vr445.bat") -Destination (Join-Path $OutDir "Start-GEVR.bat") -Force
Copy-Item -LiteralPath (Join-Path $templates "gevr-vr445-boot.cmd") -Destination (Join-Path $OutDir "gevr-vr445-boot.cmd") -Force
Copy-Item -LiteralPath (Join-Path $templates "RELEASE-NOTES-vr445.txt") -Destination (Join-Path $OutDir "RELEASE-NOTES.txt") -Force
...
# Diagnostics in the zip (declutter GitHub release page)
