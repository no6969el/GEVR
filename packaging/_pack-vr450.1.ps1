# Assemble GEVR Beta vr450.1 staging folder (BYO-ROM).
# Point release after vr450: same KEEP minus GETV_VR_GRIPUSE (needs further testing).
# Pack from getv\build-windows (or vr450 staging exe) into out\vr450.1-staging (+ zip).
...
Copy-Item -LiteralPath (Join-Path $templates "Start-GEVR-vr450.1.bat") -Destination (Join-Path $OutDir "Start-GEVR.bat") -Force
Copy-Item -LiteralPath (Join-Path $templates "gevr-vr450.1-boot.cmd") -Destination (Join-Path $OutDir "gevr-vr450.1-boot.cmd") -Force
Copy-Item -LiteralPath (Join-Path $templates "RELEASE-NOTES-vr450.1.txt") -Destination (Join-Path $OutDir "RELEASE-NOTES.txt") -Force
