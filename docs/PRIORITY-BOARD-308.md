...
> `PRIORITY-BOARD-306` and `PRIORITY-BOARD-299`.**
>
...
# §D — HOLES. **13 OF 19 CLOSED** — `H1` and `H18` closed together by `RUN-SHEET-308` §1; `H19` is new.
...
| **►► H19 (NEW, `308`)** | **`S3b` UNDER-READS BY ~10-13%, GROWING WITH DISPARITY MAGNITUDE, AND NOTHING ON THE BOARD KNOWS WHY** | **LOW** offline (`RUN-SHEET-308` §2a); **MEDIUM** if it needs the run | `[MEASURED]` `307` §2 / `308` §6: ~5 px error at −11 px predicted, ~52 px at −406 px, sign correct throughout, correlation strong. **Candidate diagnosis: the correlator averages the central quarter (`stereo_disparity.py:86`) while the prediction uses the CENTRE-RAY depth, so the two are disparities of two different depths.** `RUN-SHEET-308` §2a adds `--patch FRAC` and re-scores; **if the error does not fall as the patch tightens, it is projection scale or the log's own `D`, and that outranks everything else on the board.** `301` §0 -- a diagnosis, not a patch |
...
**UPDATED 2026-09-04 — `RUN-SHEET-308` §1 IS WORN AND `H1`/`H18` ARE CLOSED.** `308`.

0. **►►►►►►►► `RUN-SHEET-308` §2a — THE `--patch FRAC` QUESTION. OFFLINE, NO RUN, THE CORPUS IS ON DISK.** It is the only thing standing between `V3` and a real answer, and its gate is stated in advance: **the proportional error falls toward ±5 px as the patch tightens (patch-averaging, fixable in the instrument) or it does not move (projection scale or the log's `D`, and §2b is required).**
0a. **§2b — one flat-wall single-depth run — ONLY if §2a is inconclusive.** The only person-cost step on the sheet.
0b. **AND IF `V3` CLOSES, `V4` (`S4`, the mode-2 falsifier) RUNS IN THE SAME SITTING** — a pass with no negative arm is a claim, not a result (`49`).
