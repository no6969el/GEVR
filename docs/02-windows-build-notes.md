...
installer exits with code 2. Winget does NOT add LLVM to PATH â€” add
...
## GOTCHA 1 â€” CMake 4 rejects this project
...
## GOTCHA 2 â€” freetype-windows-binaries is declared but never committed
...
toolset and we link with VS2022 (v144). Fallback if the linker objects â€” switch
`FREETYPE_LIBRARIES` to `release dll/x64/freetype.lib` and copy `freetype.dll` beside the exe.

## GOTCHA 4 â€” target `N64Recomp` is a LIBRARY, not the executable
...
With Ninja on Linux, `ninja N64Recomp` resolves to the output *file* and builds the exe â€” which
...
Generator-dependent target resolution â€” expect the same trap for any other target that script names.

## GOTCHA 5 â€” vendored dxc has no Windows binaries
...
Version matters. Their vendored `inc/dxcapi.h` is sha256 `f8487a55â€¦`, which matches dxc-bin
commits `0cffb90` (v1.8.2407) through HEAD `7810655` â€” NOT the `cc15e71` (v1.8.2403.2) that
upstream RT64 pins. So use **dxc-bin HEAD**, whose headers match. (Their Linux binaries are
custom rebuilds and don't match any dxc-bin commit â€” irrelevant, since headers determine ABI.)
...
## GOTCHA 6 â€” rsp/aspMain.cpp is generated, and the standard build path never generates it
...
## GOTCHA 7 â€” GCC warning flags are passed unconditionally to MSVC
...
Fix â€” wrap each in a generator expression (6 replacements across the 3 files, backups `.bak2`):
...
## GOTCHA 8 â€” `mupen64plus-win32-deps` (SDL) is not vendored either

RT64's Windows build fails with `Cannot open include file: 'SDL.h'`. Same cause as GOTCHA 5 â€”
...
## GOTCHA 9 â€” `__attribute__((weak))` does not exist in MSVC (blocked the whole build)
...
to "GNU ld, lld, and Apple ld64" â€” MSVC is not on that list, and it doesn't parse the
...
This affects all **38** weakened functions â€” upstream's own 36 as well as our 2 â€” so it is a
fundamental upstream Windows incompatibility, not something the VR bind test introduced.

**Fix â€” rename instead of weaken.** If the recompiled original is renamed, there is only one
...
â†’ RECOMP_FUNC void NAME__recomp_orig(uint8_t* rdram, recomp_context* ctx)
```
...
The renamed originals stay in the binary, unreferenced â€” harmless, and useful later if a patch
ever wants to call through to vanilla behaviour.

## Files placed on the rig

- `ge007.tlbfree.z64` (repo root) â€” sha1 `e6e7bbcd4e16c8d83fbc86765aad9579a07d068c`, converted
  in the cloud from the user's own ROM; means xdelta3 is NOT needed on Windows.
- `patches/ge_vr_bindtest.c` â€” the Phase 0 bind test.

## Checkpoints hit on the rig (all matched the cloud run exactly)

- `.\N64Recomp.exe us.toml` â†’ **`Function count: 3150`**, same three `[Info]` lines.
- `py tools_weaken_patched.py` â†’ **`weakened 38 patched functions across 31 files`** +
...
- `PatchesBin` â€” the bind test compiled to MIPS with clang 18 and linked into `patches.elf`;
...
2. SDL2 2.26.3 (RT64) vs 2.30.3 (the DLL copied beside the exe) â€” GOTCHA 8.
3. CMake runs `./N64Recomp patches.toml` with a Unix-style path â€” may not resolve on Windows.
