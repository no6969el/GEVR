> # ►►►► SUPERSEDED 2026-09-04 BY `PRIORITY-BOARD-307`. KEPT IN FULL, RULE `2`. ◄◄◄◄
>
> **`306` wrote this board OFFLINE, before `RUN-SHEET-306` was run.** The sheet came back the same
> day and moved three of its items, so `PRIORITY-BOARD-307` is the current one: **`V1`/`V2` done,
> `V3` half answered — GATE 6.1 passed for the first time ever and GATE 6.2 is unscorable at that
> venue — plus `H17` and `H18`.**
>
> **►► THE CONVENTION FROM HERE: THE BOARD IS RE-NUMBERED BY EVERY SESSION THAT CHANGES IT, SO THE
> NUMBER IS THE DATE STAMP.** A board edited in place through seven sessions is what made `299`
> unreadable, and the fix is not to edit less — it is to number every edit.

# PRIORITY BOARD — `306`, WRITTEN 2026-09-04. **SUPERSEDES `PRIORITY-BOARD-299`.**
...
| measurement holes closed | 0 of 10 | **11 of 16** |
...
| **`258` STAGE 6's exit gate (`S3`, separation)** | **never run** | **STILL NEVER RUN — and its standing objection lapsed on 2026-09-03** |
...
| **E3** | **The far clamp: FC3/FC4's verdict.** `RUN-SHEET-306` §5 | **FREE** | **One sentence from the owner and nothing else.** If the arms looked the same again, `RUN-SHEET-303` §0.1 is a `279` §7 null for the SECOND time and closes |
...
# §D — HOLES. **11 OF 16 CLOSED.**

**CLOSED:** `H1` `H2` `H3` `H5` `H6` `H7` `H9` `H10` `H13` `N1` `N2` — the account is `301` §1,
...
1. **`RUN-SHEET-306` §1 and §2** — offline, minutes, and nothing in `305` is quotable until they pass.
2. **§3 — `V1`, read the `P1` arm.** The cheapest unread evidence on the board.
3. **§5 — `E3`, the FC3/FC4 sentence.** Free, and it has been owed for two sessions.
4. **§6 — `V3`, `S3`.** **The gate this whole stage exists to pass.**
5. **`V7`/GROUP `C`** — the per-eye contents instrument, when a session has room for a `HIGH`.
