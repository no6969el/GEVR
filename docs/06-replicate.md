...
# dxc Windows binaries — vendored copy has Linux/macOS only. Use dxc-bin HEAD.
...
```powershell
# Build the recompiler tools (note: N64RecompCLI, NOT N64Recomp — see GOTCHA 4)
...
.\RSPRecomp.exe aspMain.us.toml     # generates rsp/aspMain.cpp — GOTCHA 6
...
| `RecompiledFuncs` compile | No `C2143`/`C2082`/`C2091` — those mean weak symbols survived |
